...
import { EtherbaseUpgradeable, EtherController } from "../typechain";
...
describe("EtherbaseUpgradeable", () => {
    ...
    let etherbase: EtherbaseUpgradeable;
    const amount = ethers.utils.parseEther("5");

    beforeEach(async() => {
        [ schainOwner, hacker ] = await ethers.getSigners();
        etherbase = await (await ethers.getContractFactory('EtherbaseUpgradeable')).deploy() as EtherbaseUpgradeable;
